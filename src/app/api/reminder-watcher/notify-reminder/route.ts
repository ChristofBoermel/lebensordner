...
interface NotifyReminderWatcherRequest {
...
export async function POST(request: Request) {
  try {
    ...
    const {
      ...
      watcherEmail,
      watcherName,
    } = await request.json() as NotifyReminderWatcherRequest

    if (!reminderTitle || !watcherEmail || !watcherName) {
          ...
          <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background-color: #f0fdf4; border: 1px solid #86efac; padding: 20px; border-radius: 8px; margin: 24px 0;">
              ...
              <table style="width: 100%; color: #374151;">
                <tr>
                  <td style="padding: 4px 0; font-weight: 500;">Erinnerung:</td>
                  <td style="padding: 4px 0;">${reminderTitle}</td>
                </tr>
                ${reminderDescription ? `
                <tr>
                  <td style="padding: 4px 0; font-weight: 500;">Details:</td>
                  <td style="padding: 4px 0;">${reminderDescription}</td>
                </tr>
                ` : ''}
                <tr>
                  <td style="padding: 4px 0; font-weight: 500;">Fälligkeitsdatum:</td>
